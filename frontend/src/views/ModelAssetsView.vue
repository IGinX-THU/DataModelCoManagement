...
const uploadForm = reactive({ name: '', version: '', type: 'Python', file: null })

const handleUploadFile = (e) => {
    uploadForm.file = e.target.files[0]
    if (uploadForm.file) uploadForm.name = uploadForm.file.name
}
...
const startUpload = () => {
    ...
    const interval = setInterval(() => {
        ...
        if (p >= 100) {
            ...
            setTimeout(() => {
                // Call store upload action here
                // For now just simulate success
                alert('Upload Success! MD5 Verified.')
...
const editMetadata = () => {
    ...
    // Mock getting the latest version data
    ...
    metaForm.value = {
        ...
        originalVersion: currentVer.version, // Track original for update
...
const autoParseCode = () => {
    // Call store's regex parser
    // Mock content for now
    const mockContent = `
# @Input: speed (Float) - Vehicle Speed
# @Input: target_temp (Float) - Setpoint
# @Output: power (Float) - Heater Power
    `
    const { inputs, outputs } = modelStore.autoParseCode(mockContent)
    metaForm.value.inputs = inputs
    metaForm.value.outputs = outputs
    alert('Auto-parsing completed based on code comments.')
}
...
const handleDeleteModel = () => {
    if (!modelStore.selectedModel) return
    const res = modelStore.deleteModel(modelStore.selectedModel.id)
    if (res.success) {
        modelStore.showDeleteModal = false
        // Success, selection is already cleared in store
    } else {
        alert(res.msg)
    }
}
...
const handleDeleteVersion = (version) => {
    ...
    if (confirm(`Delete version ${version}?`)) {
        const res = modelStore.deleteModelVersion(modelStore.selectedModel.id, version)
        if (!res.success) {
            alert(res.msg)
        }
    }
...
const getTypeIcon = (type) => {
    const icons = { 'Python': 'ri-code-s-slash-line', 'MATLAB': 'ri-functions-line', 'FMU': 'ri-box-3-line', 'DLL': 'ri-file-code-line' }
            ...
            <div class="p-6 grid grid-cols-2 gap-6 overflow-y-auto">
                ...
                <div class="border border-gray-200 rounded p-4">
                    <h4 class="font-bold text-sm mb-4 flex justify-between">
                        ...
                        <span class="text-xs text-gray-500">{{ diffData.v1.uploadTime }}</span>
                    </h4>
                    <div class="space-y-4">
                        <div>
                ...
                <div class="border border-gray-200 rounded p-4">
                    <h4 class="font-bold text-sm mb-4 flex justify-between">
                        ...
                        <span class="text-xs text-gray-500">{{ diffData.v2.uploadTime }}</span>
                    </h4>
                    <div class="space-y-4">
                        <div>
                            <span class="text-xs font-bold text-gray-500 block mb-1">Inputs</span>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select v-model="uploadForm.type" class="w-full border border-gray-300 rounded px-2 py-1.5 text-xs">
                                <option>Python</option><option>MATLAB</option><option>FMU</option><option>DLL</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-500 mb-1">Version</label>
                            <input v-model="uploadForm.version" class="w-full border border-gray-300 rounded px-2 py-1.5 text-xs" placeholder="v1.0.0">
                        </div>
                    </div>
        ...
        <div class="bg-white rounded-lg shadow-xl w-[700px] flex flex-col max-h-[85vh]">
            ...
            <div class="p-6 overflow-y-auto space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-500 mb-1">Version</label>
                        <input v-model="metaForm.version" class="w-full border border-gray-300 rounded px-2 py-1.5 text-xs">
                    </div>
                    <div class="col-span-2">
            ...
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-2 mt-1">
                        <span class="px-2 py-0.5 bg-blue-50 text-blue-600 rounded text-xs font-medium border border-blue-100">{{ modelStore.selectedModel.type }}</span>
                ...
                <div class="col-span-2 space-y-6">
                    <div>
                        ...
                        <p class="text-sm text-gray-600 leading-relaxed bg-gray-50 p-4 rounded border border-gray-100">
                            This model implements a standard PID control loop for vehicle temperature management. 
                            It accepts real-time sensor inputs and outputs control signals for the actuator.
                        </p>
                    </div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <div class="bg-gray-50 rounded border border-gray-100 p-4 space-y-2 text-xs">
                            ...
                            <div class="flex justify-between"><span class="text-gray-500">Size:</span> <span class="text-gray-700">{{ modelStore.selectedModel.size }}</span></div>
                            <div class="flex justify-between"><span class="text-gray-500">Uploaded:</span> <span class="text-gray-700">{{ modelStore.selectedModel.uploadTime }}</span></div>
                            <div class="flex justify-between"><span class="text-gray-500">References:</span> <span class="text-gray-700">{{ modelStore.selectedModel.refCount }} tasks</span></div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-2 max-h-48 overflow-y-auto">
                            <div v-for="v in modelStore.selectedModel.history" :key="v.version" class="flex items-center text-xs group hover:bg-gray-50 p-1 rounded">
                                ...
                                <span class="text-gray-400 mr-2">{{ v.uploadTime.split(' ')[0] }}</span>
